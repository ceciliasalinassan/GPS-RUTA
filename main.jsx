...
const aiData=useMemo(()=>{
 ...
 const sugerencias=[
  ...
  riesgoCliente.filter(c=>c.riesgo==="ALTO").length?`Revisar clientes de riesgo ALTO: 4 o más facturas vencidas.`:`Clientes PREMIUM detectados: ${riesgoCliente.filter(c=>c.riesgo==="PREMIUM").length}`
 ];
...
function Table({rows,type,setData,data}){let sorted=[...rows].sort((a,b)=>(b.fecha||b.vencimiento).localeCompare(a.fecha||a.vencimiento));return <div className="tableWrap"><table><thead><tr><th>Fecha</th><th>Mes/Año</th><th>Categoría</th><th>Descripción</th><th>Monto</th><th>Acciones</th></tr></thead><tbody>{sorted.map(r=><tr key={r.id}><td>{r.fecha||r.vencimiento}</td><td>{ml(mk(r.fecha||r.vencimiento))}</td><td>{r.categoria}{r.proveedor&&<small>{r.proveedor}</small>}{r.emailProveedor&&<small>{r.emailProveedor}</small>}</td><td>{r.descripcion}{r.estado&&<small>Estado: {r.estado}</small>}</td><td>{money(r.monto)}</td><td>{type==="debts"&&<a className="icon mail" href={debtEmail(r)} title="Enviar correo deuda"><Mail size={17}/></a>}<button className="icon trash" onClick={()=>setData({...data,[type]:(data[type]||[]).filter(x=>x.id!==r.id)})}><Trash2 size={17}/></button></td></tr>)}</tbody></table></div>}

...
const[clientForm,setClientForm]=useState({nombre:"",rut:"",giro:"",telefono:"569",email:"",direccion:"",contacto:""}),[invoiceForm,setInvoiceForm]=useState({clienteId:"",factura:"",emision:today(),vencimiento:today(),monto:"",estado:"Pendiente",detalle:""}),[incomeForm,setIncomeForm]=useState({fecha:today(),categoria:"Pago de factura",descripcion:"",monto:"",facturaId:""}),[expenseForm,setExpenseForm]=useState({fecha:today(),categoria:"Pago instalador",descripcion:"",monto:"",debtId:"",numeroFacturaPago:""}),[debtForm,setDebtForm]=useState({fecha:today(),proveedor:"",categoria:"Compra de equipos",descripcion:"",monto:"",vencimiento:today(),estado:"Pendiente"}),[editingClient,setEditingClient]=useState(null),[editingInvoice,setEditingInvoice]=useState(null);
...
function saveDebt(){if(!debtForm.proveedor||!debtForm.monto)return;setData({...data,debts:[{...debtForm,estado:"Pendiente",id:Date.now(),monto:+debtForm.monto},...data.debts]});setDebtForm({fecha:today(),proveedor:"",categoria:"Compra de equipos",descripcion:"",monto:"",vencimiento:today(),estado:"Pendiente"})}
...
<div className="historyHead"><h2>Listado de facturas por cobrar</h2><small className="requiredAttach">Recordatorio no requiere adjunto. Enviar factura sí requiere adjuntar la factura.</small></div>
<InvTable items={filteredInvoices} client={client} edit={editInvoice} del={id=>setData({...data,invoices:data.invoices.filter(i=>i.id!==id)})} data={data} attachFile={attachFile} canSendInvoice={canSendInvoice}/>
</div></section>}
